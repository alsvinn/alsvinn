                          ...
                          const grid::Grid &grid)
{
    ...
    for (const std::string& variableName : variables) {
        if (conservedVolumeIn.hasVariable(variableName)) {
            ...
            for (int k = 0; k < innerSize.z; ++k) {
                for (int j = 0; j < innerSize.y; ++j) {
                    for (int i = 0; i < innerSize.x; ++i) {
                        const real value = (viewIn.at(i+ghostCells.x, j+ghostCells.y, k + ghostCells.z)-minValue)/(maxValue-minValue);
            ...
            THROW("Unknown variable name given to LegendrePointWise functional: " << variableName);
        }
    }
