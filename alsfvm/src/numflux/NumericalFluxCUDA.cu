...
namespace alsfvm { namespace numflux { 
	...
	namespace {
		...
		__global__ void computeFluxDevice(typename Equation::ConstViews left, typename Equation::ConstViews right, typename Equation::Views output,
			...
			Equation::addToViewAt(output, index, cellScaling*(fluxMiddleRight)-cellScaling*(fluxLeftMiddle));
		}
		...
		void computeFlux(const volume::Volume& left, const volume::Volume& right, volume::Volume& output, size_t numberOfGhostCells, real cellScaling) {
			...
			const size_t numberOfXCells = left.getTotalNumberOfXCells() - 2 * numberOfGhostCells;
			const size_t numberOfYCells = left.getTotalNumberOfYCells() - 2 * (dimension > 1) * numberOfGhostCells;
			const size_t numberOfZCells = left.getTotalNumberOfZCells() - 2 * (dimension > 2) * numberOfGhostCells;
