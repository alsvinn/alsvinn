...
void NetCDFWriter::writeVolume(netcdf_raw_ptr baseGroup, const volume::Volume &volume, std::array<netcdf_raw_ptr, 3> dimensions)
{

    for (size_t variable = 0; variable < volume.getNumberOfVariables(); ++variable) {
...
std::pair<netcdf_raw_ptr, netcdf_raw_ptr> NetCDFWriter::makeDataset(netcdf_raw_ptr baseGroup, const volume::Volume &volume, size_t memoryIndex,
                                                                    std::array<netcdf_raw_ptr, 3> dimensions)
{
