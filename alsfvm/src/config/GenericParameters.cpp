...
double GenericParameters::getDouble(const std::string &key, double defaultValue) const
{
...
int GenericParameters::getInteger(const std::string &key, int defaultValue) const
{
    if(tree.find(key) != tree.not_found()) {
