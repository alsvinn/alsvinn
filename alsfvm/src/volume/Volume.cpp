...
///
alsfvm::shared_ptr<memory::Memory<real> >& Volume::getScalarMemoryArea(
    size_t index) {
...
/// \return the MemoryArea for the given index
///
alsfvm::shared_ptr<const memory::Memory<real> > Volume::getScalarMemoryArea(
    size_t index) const {
    return memoryAreas[index];
}
...
void Volume::copyInternalCells(size_t memoryAreaIndex, real* output,
    size_t outputSize) const {
    memoryAreas[memoryAreaIndex]->copyInternalCells(numberOfXGhostCells,
        getTotalNumberOfXCells() - numberOfXGhostCells,
        numberOfYGhostCells, getTotalNumberOfYCells() - numberOfYGhostCells,
        numberOfZGhostCells, getTotalNumberOfZCells() - numberOfZGhostCells, output,
        outputSize);
}
...
void Volume::copyTo(volume::Volume& other) const {
    ...
    for (size_t var = 0; var < getNumberOfVariables(); ++var) {
        getScalarMemoryArea(var)->copyToHost(temporaryStorage.data(),
            temporaryStorage.size());
        other.getScalarMemoryArea(var)->copyFromHost(temporaryStorage.data(),
            temporaryStorage.size());
    }
}
...
alsfvm::shared_ptr<const memory::Memory<real> > Volume::operator[](
    size_t index) const {
...
void Volume::addLinearCombination(real a1, real a2, const Volume& v2, real a3,
    const Volume& v3, real a4, const Volume& v4, real a5, const Volume& v5) {
    CHECK_SIZE_THIS(v2);
    CHECK_SIZE_THIS(v3);
    CHECK_SIZE_THIS(v4);
    CHECK_SIZE_THIS(v5);
    ...
    for (size_t i = 0; i < memoryAreas.size(); ++i) {
        this->getScalarMemoryArea(i)->addLinearCombination(a1, a2, *v2[i], a3, *v3[i],
            a4, *v4[i], a5, *v5[i]);
    }
}
...
std::shared_ptr<Volume> Volume::makeInstance(size_t nxNew, size_t nyNew,
    size_t nzNew, const std::string& platform) const {
    if (platform == "default" || platform == memoryFactory->getPlatform()) {
        return std::make_shared<Volume>(variableNames, memoryFactory,
                nxNew, nyNew, nzNew, 0);
    } else {
        alsfvm::shared_ptr<DeviceConfiguration> deviceConfiguraiton(
        ...
        return std::make_shared<Volume>(variableNames, memoryFactoryForPlatform, nxNew,
                nyNew, nzNew, 0);
    }
