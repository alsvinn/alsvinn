...
__global__ void applyBoundaryConditionsDevice(memory::View<real> memoryArea,
    ...
    const size_t internalNumberOfXCells, const size_t internalNumberOfYCells,
    const size_t internalNumberOfZCells, const size_t numberOfGhostCells) {
    ...
    const size_t yInternalFormat = yDir ? 0 : (index / numberOfXCells) %
        numberOfYCells;
    const size_t zInternalFormat = zDir ? 0 : min(int((index) /
                (numberOfXCells * numberOfYCells)),
            int(numberOfZCells - 1));


    const size_t x = xInternalFormat + xDir * (numberOfGhostCells +
            internalNumberOfXCells * top);
    const size_t y = yInternalFormat + yDir * (numberOfGhostCells +
            internalNumberOfYCells * top);
    const size_t z = zInternalFormat + zDir * (numberOfGhostCells +
            internalNumberOfZCells * top);

    for (size_t ghostCell = 1; ghostCell <= numberOfGhostCells; ++ghostCell) {
        BoundaryConditions::applyBoundary(memoryArea, x, y, z, ghostCell,
            numberOfGhostCells, top, xDir, yDir, zDir);
    }
}

template<class BoundaryConditions, bool top, bool xDir, bool yDir, bool zDir>
void applyBoundaryConditions(memory::View<real>& memoryArea,
    ...
    const size_t internalNumberOfXCells = memoryArea.getNumberOfXCells() - 2 *
        numberOfGhostCells - 1;
    const size_t internalNumberOfYCells = memoryArea.getNumberOfYCells() - 2 *
        numberOfGhostCells - 1;
    const size_t internalNumberOfZCells = memoryArea.getNumberOfZCells() - 2 *
        numberOfGhostCells - 1;
...
void callApplyBoundaryConditions(memory::View<real>& memoryArea,
    size_t numberOfGhostCells,
    const grid::Grid& grid) {

    if (grid.getBoundaryCondition(0) != MPI_BC) {
        applyBoundaryConditions<BoundaryConditions, 0, 1, 0, 0>(memoryArea,
            numberOfGhostCells);
    }

    if (grid.getBoundaryCondition(1) != MPI_BC) {
        applyBoundaryConditions<BoundaryConditions, 1, 1, 0, 0>(memoryArea,
            numberOfGhostCells);
    }

    if (memoryArea.getNumberOfYCells() > 1) {
        if (grid.getBoundaryCondition(2) != MPI_BC) {
            applyBoundaryConditions<BoundaryConditions, 0, 0, 1, 0>(memoryArea,
                numberOfGhostCells);
        }

        if (grid.getBoundaryCondition(3) != MPI_BC) {
            applyBoundaryConditions<BoundaryConditions, 1, 0, 1, 0>(memoryArea,
                numberOfGhostCells);
        }
    }
    ...
    if (memoryArea.getNumberOfZCells() > 1) {
        if (grid.getBoundaryCondition(4) != MPI_BC) {
            applyBoundaryConditions<BoundaryConditions, 0, 0, 0, 1>(memoryArea,
                numberOfGhostCells);
        }

        if (grid.getBoundaryCondition(5) != MPI_BC) {
            applyBoundaryConditions<BoundaryConditions, 1, 0, 0, 1>(memoryArea,
                numberOfGhostCells);
        }
    }
}
...
///
template<class BoundaryConditions>
void BoundaryCUDA < BoundaryConditions > :: applyBoundaryConditions(
