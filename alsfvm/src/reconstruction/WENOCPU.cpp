...
typename std::enable_if < i == -1 >::type computeAlpha(std::array < real,
    2 * order - 1 > & stencil,
...
typename std::enable_if < i != -1 >::type computeAlpha(
    ...
    right[i] = WENOCoefficients<order>::coefficients[i] / std::pow(beta + epsilon,
            2);
    sumRight += right[i];

    left[i] = WENOCoefficients<order>::coefficients[order - 1 - i] / std::pow(
            beta + epsilon, 2);
    sumLeft += left[i];
...
template<int order> WENOCPU<order>::WENOCPU() {
    //empty
}
...
void WENOCPU<order>::performReconstruction(const volume::Volume& inputVariables,
    ...
    const real* pointerInWeight = inputVariables.getScalarMemoryArea(
            indicatorVariable)->getPointer();
        ...
        for (size_t y = startY; y < endY; y++) {
            ...
            for (int x = startX; x < int(endX); x++) {
                ...
                static_assert(order < 4
                    && order > 1, "So far, we only support order = 2 or order = 3");
