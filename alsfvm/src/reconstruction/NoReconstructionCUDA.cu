    ...
    void NoReconstructionCUDA::performReconstruction(const volume::Volume& inputVariables,
		...
		for (size_t var = 0; var < inputVariables.getNumberOfVariables(); ++var) {
			CUDA_SAFE_CALL(cudaMemcpyAsync(leftOut.getScalarMemoryArea(var)->getPointer(), inputVariables.getScalarMemoryArea(var)->getPointer(),
				leftOut.getScalarMemoryArea(var)->getSize() * sizeof(real), cudaMemcpyDeviceToDevice));
			CUDA_SAFE_CALL(cudaMemcpyAsync(rightOut.getScalarMemoryArea(var)->getPointer(), inputVariables.getScalarMemoryArea(var)->getPointer(),
				rightOut.getScalarMemoryArea(var)->getSize() * sizeof(real), cudaMemcpyDeviceToDevice));
		}
	}
