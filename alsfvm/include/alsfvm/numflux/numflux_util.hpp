...
-> typename std::enable_if<has_stencil<NumericalFluxType>::value, decltype(NumericalFluxType::stencil())>::type 
{
...
__device__ __host__ 
typename std::enable_if<!has_stencil<NumericalFluxType>::value, ivec2>::type
getStencil(NumericalFluxType) {
    return { 0,1 };
}
    ...
    typename Equation::ConservedVariables& out)
{
    ...
    // 
    typename Equation::ConservedVariables fluxMiddleRight;
    real waveSpeed = Flux::template computeFlux<direction>(eq, u0, u1, u2, u3, fluxMiddleRight);
