...
public:
    ...
    __device__ __host__ inline static real computeFlux(const
        ...
        if (speedLeft == 0) {
            ...
            F = (speedRight * leftFlux - speedLeft * rightFlux + speedRight * speedLeft *
                    (right.conserved() - left.conserved())) / (speedRight - speedLeft);
        }
    ...
    __device__ __host__ inline static void computeHLLSpeeds(
        ...
        const real p = (left.p * waveLeft + right.p * waveRight) /
            (waveLeft + waveRight);

        const real cs = sqrt(eq.getGamma() * p / rho);

        speedLeft = fmin(left.u[direction] - sqrt(eq.getGamma() * left.p / left.rho),
                u[direction] - cs);
        speedRight = fmax(right.u[direction] + sqrt(eq.getGamma() * right.p /
                    right.rho), u[direction] + cs);
