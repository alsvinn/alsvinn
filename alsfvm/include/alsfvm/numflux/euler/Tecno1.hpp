...
template<>
template<int direction>
__device__ __host__ inline  real Tecno1<3>::computeFlux(
    const equation::euler::Euler<3>& eq,
    ...
    if (left == right) {
        ...
        F[1] = bar(leftZ[4], rightZ[4]) / bar(leftZ[0], rightZ[0]) + bar(leftZ[1],
                rightZ[1]) / bar(leftZ[0], rightZ[0]) * F[0];
        F[2] = bar(leftZ[2], rightZ[2]) / bar(leftZ[0], rightZ[0]) * F[0];
        F[3] = 0; // assume 2D
        F[4] = 1.0 / (2 * bar(leftZ[0],
        ...
        F[2] = bar(leftZ[4], rightZ[4]) / bar(leftZ[0], rightZ[0]) + bar(leftZ[2],
    ...
    real cs = 0;
    HLL3<3>::computeHLLSpeeds<direction>(eq, left, right, leftSpeed, rightSpeed,
        cs);
...
template<>
template<int direction>
__device__ __host__ inline  real Tecno1<2>::computeFlux(
    const equation::euler::Euler<2>& eq,
    ...
    if (left == right) {
        ...
        F[1] = bar(leftZ[3], rightZ[3]) / bar(leftZ[0], rightZ[0]) + bar(leftZ[1],
                rightZ[1]) / bar(leftZ[0], rightZ[0]) * F[0];
        F[2] = bar(leftZ[2], rightZ[2]) / bar(leftZ[0], rightZ[0]) * F[0];
        F[3] = 1.0 / (2 * bar(leftZ[0],
                    rightZ[0])) * ((gamma + 1) / (gamma - 1) * divLn(leftZ[0], rightZ[0],
                    (F[0])) + bar(leftZ[1], rightZ[1]) * F[1] + bar(leftZ[2], rightZ[2]) * F[2]);
    } else  if (direction == 1) {
    ...
    real leftSpeed = 0, rightSpeed = 0;
    real cs = 0;
    HLL3<2>::computeHLLSpeeds<direction>(eq, left, right, leftSpeed, rightSpeed,
        cs);
    return fmax(fabs(leftSpeed), fabs(rightSpeed));
...
template<>
template<int direction>
__device__ __host__ inline  real Tecno1<1>::computeFlux(
    const equation::euler::Euler<1>& eq,
    ...
    if (left == right) {
        ...
        F[1] = bar(leftZ[2], rightZ[2]) / bar(leftZ[0], rightZ[0]) + bar(leftZ[1],
                rightZ[1]) / bar(leftZ[0], rightZ[0]) * F[0];
        F[2] = 1.0 / (2 * bar(leftZ[0],
                    rightZ[0])) * ((gamma + 1) / (gamma - 1) * divLn(leftZ[0], rightZ[0],
                    (F[0])) + bar(leftZ[1], rightZ[1]) * F[1]);
    }

    real leftSpeed = 0, rightSpeed = 0;
    real cs = 0;
    HLL3<1>::computeHLLSpeeds<direction>(eq, left, right, leftSpeed, rightSpeed,
        cs);
    return fmax(fabs(leftSpeed), fabs(rightSpeed));
