...
public:
    ...
    __device__ __host__ inline static real computeFlux(const
        ...
        real sm = (right.m[direction] * udr - left.m[direction] * udl + pressureRight -
                pressureLeft) / aa;

        rvec us = ( fluxRight.m - fluxLeft.m - speedRight * right.m + speedLeft *
                left.m ) / aa;
        us[direction] = sm;
        ...
        if (speedLeft == 0) {
            ...
            middle.E = (udl * left.E + pressureLeft * left.u[direction] - p * sm) /
                (sm - speedLeft);
            ...
            middle.E = (udr * right.E + pressureRight * right.u[direction] - p * sm) /
                (sm - speedRight);
    ...
    ///
    template<int direction>
    __device__ __host__ inline static void computeHLLSpeeds(
        const equation::euler::Euler<nsd>& eq,
        const equation::euler::AllVariables<nsd>& left,
        const equation::euler::AllVariables<nsd>& right, real& speedLeft,
        real& speedRight, real& cs) {

        const real waveLeft = sqrt(left.rho);
        const real waveRight = sqrt(right.rho);

        const real rho = (left.rho + right.rho) / 2;
        const rvec u = (waveLeft * left.u + waveRight * right.u) /
            (waveLeft + waveRight);
        const real p = (waveLeft * left.p + waveRight * right.p) /
            (waveLeft + waveRight);
        ...
        const real correct = 0.5 * fmax(real(0),
                left.u[direction] - right.u[direction]);
