...
class Euler {
    public:
        ...
        typedef equation::euler::Views<const volume::Volume, const memory::View<const real>, nsd >
        ConstViews;

        typedef equation::euler::ViewsExtra<volume::Volume, memory::View<real>, nsd >
        ViewsExtra;
        typedef equation::euler::ViewsExtra<const volume::Volume, const memory::View<const real>, nsd >
        ConstViewsExtra;
        ...
        __device__ __host__ static ConservedVariables fetchConservedVariables(
            euler::Views<T, S, nsd>& views, size_t index) {
        ...
        ///
        __device__ __host__ void addToViewAt(Views& output, size_t index,
            const ConservedVariables& input) const {
            output.rho.at(index) += input.rho;
            output.m(index) += input.m;
            output.E.at(index) += input.E;
        ...
        //! \endcode
        template<size_t direction>
        __device__ __host__ ConservedVariables computePointFlux(
            const ConservedVariables& u) const {
        ...
        __device__ __host__ AllVariables makeAllVariables(real rho, vec m,
            real E) const {
        ...
        //!
        __device__ __host__ TecnoVariables computeTecnoVariables(
            const ConservedVariables& conserved) const {
        ...
        ///
        __device__ __host__ state_vector computeEntropyVariables(
            const ConservedVariables& conserved) const {
            auto primitive = computePrimitiveVariables(conserved);
            const real s = log(primitive.p) - gamma * log(conserved.rho);

            return state_vector((gamma - s) / (gamma - 1) - (conserved.rho *
                        (primitive.u.dot(primitive.u))) / (2 * primitive.p),
        ...
        ///
        __device__ __host__ vec computeEntropyPotential(const ConservedVariables&
            conserved) const {
            return conserved.m;
        }
        ...
        template<int direction>
        __device__ __host__ state_vector
        computeEntropyVariablesMultipliedByEigenVectorMatrix(const ConservedVariables&
            conserved) const {

            return computeEigenVectorMatrix<direction>(conserved).transposed() *
                computeEntropyVariables(conserved);
        }


        //! Computes the Eigen vector matrix. See 3.2.2 for full description in http://www.springer.com/de/book/9783540252023
        template<int direction>
        __device__ __host__ state_matrix computeEigenVectorMatrix(
            const ConservedVariables& conserved) const;
        ...
        template<int direction>
        __device__ __host__ state_vector computeEigenValues(const ConservedVariables&
            conserved) const;
    private:
        const real gamma;
};
...
__device__ __host__ matrix5 Euler<3>::computeEigenVectorMatrix(
    const ConservedVariables& conservedConst) const {
    auto conserved = conservedConst;

    if (direction == 1) {
        // We use the rotation trick, see 3.2.2 and Proposition 3.19 in Toro's book
        // http://www.springer.com/de/book/9783540252023
        conserved = ConservedVariables(conserved.rho, rvec3{conserved.m.y, -conserved.m.x, conserved.m.z},
                conserved.E);

    } else if (direction == 2) {
        // We use the rotation trick, see 3.2.2 and Proposition 3.19 in Toro's book
        // http://www.springer.com/de/book/9783540252023
        conserved = ConservedVariables(conserved.rho, rvec3{conserved.m.z, conserved.m.y, -conserved.m.x},
                conserved.E);
    }
...
template<>
template<int direction>
__device__ __host__ rvec5 Euler<3>::computeEigenValues(const ConservedVariables&
    conservedConst) const {
    auto conserved = conservedConst;
    ...
    if (direction < 3) {
        auto primitive = computePrimitiveVariables(conserved);
        const real a = sqrt(gamma * primitive.p / conserved.rho);
        return rvec5(primitive.u.x - a, primitive.u.x, primitive.u.x, primitive.u.x,
                primitive.u.x + a);
    }
...
template<>
template<int direction>
__device__ __host__ matrix4 Euler<2>::computeEigenVectorMatrix(
    const ConservedVariables& conservedConst) const {


    ConservedVariables conserved = conservedConst;

    if (direction  == 1) {
        // We use the rotation trick, see 3.2.2 and Proposition 3.19 in Toro's book
        // http://www.springer.com/de/book/9783540252023
        conserved = ConservedVariables(conserved.rho, rvec2{conserved.m.y, -conserved.m.x},
                conserved.E);
    }
...
template<>
template<int direction>
__device__ __host__ rvec4 Euler<2>::computeEigenValues(const ConservedVariables&
    conservedConst) const {
    ConservedVariables conserved = conservedConst;
    ...
    if (direction < 2) {
        auto primitive = computePrimitiveVariables(conserved);
        const real a = sqrt(gamma * primitive.p / conserved.rho);
        return rvec4(primitive.u.x - a, primitive.u.x, primitive.u.x,
                primitive.u.x + a);
    }
...
template<>
template<int direction>
__device__ __host__ rvec3 Euler<1>::computeEigenValues(const ConservedVariables&
    conserved) const {
    if (direction == 0) {
        auto primitive = computePrimitiveVariables(conserved);
        const real a = sqrt(gamma * primitive.p / conserved.rho);
