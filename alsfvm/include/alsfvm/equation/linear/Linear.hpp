...
#include "alsfvm/equation/linear/ExtraVariables.hpp"
#include "alsfvm/equation/linear/AllVariables.hpp"
#include "alsfvm/equation/linear/PrimitiveVariables.hpp"

#include "alsfvm/equation/linear/Views.hpp"
#include "alsfvm/volume/Volume.hpp"
#include "alsfvm/equation/linear/ViewsExtra.hpp"
...
public:
    ...
    Linear(const EquationParameters& parameters) {
        // empty
    }

    typedef EquationParameters Parameters;
    typedef linear::ConservedVariables ConservedVariables;
    typedef linear::ExtraVariables ExtraVariables;
    typedef linear::PrimitiveVariables PrimitiveVariables;
    typedef linear::AllVariables AllVariables;
    ...
    /// corresponds to the "a" variable in the tecno variable for the Linear
    /// log entropy
    ///
    ...
    ///
    /// Gives the lower bound for the parameter the entropy functions,
    /// corresponds to the "b" variable in the tecno variable for the Linear
    /// log entropy
    ///
    ...
    typedef equation::linear::Views<volume::Volume, memory::View<real> > Views;
    typedef equation::linear::Views<const volume::Volume, const memory::View<const real> >
    ConstViews;

    typedef equation::linear::ViewsExtra<volume::Volume, memory::View<real> >
    ViewsExtra;
    typedef equation::linear::ViewsExtra<const volume::Volume, const memory::View<const real> >
    ConstViewsExtra;
    ...
    __device__ __host__ static  ConservedVariables fetchConservedVariables(
        linear::Views<T, S>& views, size_t index)  {
    ...
    /// \note Empty function, Linear has no extra varaibles at the moment.
    ///
