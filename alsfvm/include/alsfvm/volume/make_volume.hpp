...
namespace volume {
    ...
    inline alsfvm::shared_ptr<Volume> makeConservedVolume(const std::string& platform,
                                          const std::string& equation,
                                          const ivec3& innerSize,
                                                 const int ghostCells) {

        alsfvm::shared_ptr<DeviceConfiguration> deviceConfiguration(new DeviceConfiguration(platform));
