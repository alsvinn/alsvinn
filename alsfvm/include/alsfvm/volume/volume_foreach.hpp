...
inline void for_each_cell_index_with_neighbours(const Volume& in,
    const std::function<void(size_t leftIndex, size_t middleIndex, size_t rightIndex)>&
    function,
    ivec3 offsetStart = {0, 0, 0},
    ivec3 offsetEnd = {0, 0, 0}) {
...
///
inline void for_each_cell_index_with_neighbours(size_t direction,
    const Volume& in,
    const std::function<void(size_t leftIndex, size_t middleIndex, size_t rightIndex)>&
    function,
    ivec3 offsetStart = { 0, 0, 0 },
...
inline VariableStruct expandVariableStruct(const std::array<const real*, 0>& in,
    size_t index) {
...
template<class VariableStruct>
inline VariableStruct expandVariableStruct(const std::array<const real*, 1>& in,
    size_t index) {
...
template<class VariableStruct>
inline VariableStruct expandVariableStruct(const std::array<const real*, 3>& in,
    size_t index) {
...
template<class VariableStruct>
inline VariableStruct expandVariableStruct(const std::array<const real*, 2>& in,
    size_t index) {
    return VariableStruct(in[0][index], in[1][index]);
}

template<class VariableStruct>
inline VariableStruct expandVariableStruct(const std::array<const real*, 5>& in,
    size_t index) {
...
template<class VariableStruct>
inline VariableStruct expandVariableStruct(const std::array<const real*, 4>& in,
    size_t index) {
    return VariableStruct(in[0][index], in[1][index], in[2][index], in[3][index]);
}
...
template<class VariableStruct>
inline void saveVariableStruct(const VariableStruct& in, size_t index,
...
inline void for_each_cell(const Volume& inA, const Volume& inB,
    ...
    if (pointersInA.size() != inA.getNumberOfVariables()) {
        THROW("We expected to get " << pointersInA.size() << " variables, but got " <<
            inA.getNumberOfVariables());
    }

    if (pointersInB.size() != inB.getNumberOfVariables()) {
        THROW("We expected to get " << pointersInB.size() << " variables, but got " <<
            inB.getNumberOfVariables());
    }
...
inline void fill_volume(Volume& outA,  Volume& outB,
    ...
    if (pointersOutA.size() != outA.getNumberOfVariables()) {
        THROW("We expected to get " << pointersOutA.size() << " variables, but got " <<
            outA.getNumberOfVariables());
    }

    if (pointersOutB.size() != outB.getNumberOfVariables()) {
        THROW("We expected to get " << pointersOutB.size() << " variables, but got " <<
            outB.getNumberOfVariables());
    }
...
inline void for_each_midpoint(const Volume& volume,
    const grid::Grid& grid,
    const std::function
    < void(real x, real y, real z, size_t index, size_t i, size_t j, size_t k) >&
    function) {
    ...
    for (size_t k = ghostZ; k < nz - ghostZ; k++) {
        for (size_t j = ghostY; j < ny - ghostY; j++) {
            for (size_t i = ghostX; i < nx - ghostX; i++) {
                ...
                function(midPoint.x, midPoint.y, midPoint.z, index, i - ghostX, j - ghostY,
                    k - ghostZ);
            }
        }
    }
...
inline void for_each_midpoint(const Volume& volume,
    ...
    for_each_midpoint(volume, grid, [&](real x, real y, real z, size_t index,
...
inline void fill_volume(Volume& out,
    const grid::Grid& grid,
    const std::function < void(real x, real y, real z, VariableStruct& out) >&
    fillerFunction) {
...
inline void for_each_internal_volume_index(const Volume& volume,
    const std::function<void(size_t indexLeft, size_t indexMiddle, size_t indexRight)>&
    function,
    size_t ghostLayer) {
...
inline void for_each_internal_volume_index(const Volume& volume,
    const std::function<void(size_t indexLeft, size_t indexMiddle, size_t indexRight)>&
    function) {
    for_each_internal_volume_index<direction>(volume, function,
        volume.getNumberOfXGhostCells());
}
...
inline void for_each_internal_volume_index(const Volume& volume,
    ...
    const std::function<void(size_t indexLeft, size_t indexMiddle, size_t indexRight)>&
    function) {
    for_each_internal_volume_index(volume, direction,  function,
        volume.getNumberOfXGhostCells());
}
}
}
