...
namespace {
    struct MeanVarTestParameters {
        ...
        MeanVarTestParameters(const std::string& generatorName,
                              ...
                              int D)
            : generatorName(generatorName), distributionName(distName), mean(mean), variance(variance), D(D)
        {
...
struct MeanVarTest : public ::testing::TestWithParam <MeanVarTestParameters> {
    ...
    std::shared_ptr<alsuq::distribution::Distribution> makeDistribution(int M, int D) {
        auto parameters = GetParam();
        alsuq::distribution::DistributionFactory factory;
        return factory.createDistribution(parameters.distributionName, D, M, parameters.parameters);
    }
};
