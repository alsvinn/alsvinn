...
TEST(BurgersWenoTest, ConstantZeroTestSecondOrder) {
    ...
    auto wenoCPU = reconstructionFactory.createReconstruction("weno2", "burgers", parameters, memoryFactory,  grid, deviceConfiguration);
...
TEST(BurgersWenoTest, ReconstructionSimple) {
    ...
    ASSERT_NEAR(omega0 * right1 + omega1 * right2, right->getScalarMemoryArea("u")->getPointer()[2], 1e-8);
    ASSERT_NEAR(omega0Tilde * left1 + omega1Tilde * left2, left->getScalarMemoryArea("u")->getPointer()[2], 1e-8);
...
TEST(BurgersWenoTest, ReconstructionSimpleYDirection) {
    ...
    ASSERT_NEAR(omega0 * right1 + omega1 * right2, right->getScalarMemoryArea("u")->getPointer()[3+2*totalNx], 1e-8);
    ASSERT_NEAR(omega0Tilde * left1 + omega1Tilde * left2, left->getScalarMemoryArea("u")->getPointer()[3+2*totalNx], 1e-8);
