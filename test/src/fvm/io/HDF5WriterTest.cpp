...
TEST_F(HDF5WriterTest, WriteAndReadTest) {
    ...
    HDF5Resource lowerCornerAttribute(H5Aopen(gridGroup.hid(), "vsLowerBounds",
            H5P_DEFAULT), H5Aclose);
    std::vector<float> lowerCorner(3, 42);
    HDF5_SAFE_CALL(H5Aread(lowerCornerAttribute.hid(), H5T_NATIVE_FLOAT,
            lowerCorner.data()));
    ...
    HDF5Resource upperCornerAttribute(H5Aopen(gridGroup.hid(), "vsUpperBounds",
            H5P_DEFAULT), H5Aclose);
    std::vector<float> upperCorner(3, 42);
    HDF5_SAFE_CALL(H5Aread(upperCornerAttribute.hid(), H5T_NATIVE_FLOAT,
            upperCorner.data()));
    ...
    HDF5Resource startCellAttribute(H5Aopen(gridGroup.hid(), "vsStartCell",
            H5P_DEFAULT), H5Aclose);
    std::vector<int> startCell(3, 42);
    HDF5_SAFE_CALL(H5Aread(startCellAttribute.hid(), H5T_NATIVE_INT,
            startCell.data()));
    ...
    HDF5Resource numCellsAttribute(H5Aopen(gridGroup.hid(), "vsNumCells",
            H5P_DEFAULT), H5Aclose);
    std::vector<int> numCells(3, 42);
    HDF5_SAFE_CALL(H5Aread(numCellsAttribute.hid(), H5T_NATIVE_INT,
            numCells.data()));
