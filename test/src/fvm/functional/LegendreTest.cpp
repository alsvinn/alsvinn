...
TEST(LegendreTest, ConstantTest) {
    ...
    ivec3 size = {3,2,1};
    auto conservedIn = alsfvm::volume::makeConservedVolume("cpu", "euler2", size, 2);

    grid::Grid grid({0,0,0}, {1,1,1}, size);
    volume::fill_volume<equation::euler::ConservedVariables<2> >(*conservedIn, grid, [](real x, real y, real z, equation::euler::ConservedVariables<2>& out) {
    ...
    auto conservedOut = alsfvm::volume::makeConservedVolume("cpu", "euler2", {1,1,1}, 2);
    conservedOut->makeZero();
    auto extraIn = alsfvm::volume::makeExtraVolume("cpu", "euler2", size, 2);

    volume::fill_volume<equation::euler::ExtraVariables<2> >(*extraIn, grid, [](real x, real y, real z, equation::euler::ExtraVariables<2>& out) {
    ...
    extraOut->makeZero();
    functional->operator ()(*conservedOut, *extraOut, *conservedIn, *extraIn, 0.5, grid);
