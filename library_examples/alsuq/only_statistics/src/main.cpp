...
void addWriters(alsuq::stats::StatisticsFactory::StatisticsPointer statisticsPointer,
  ...
  for (auto statsName :  statisticsPointer->getStatisticsNames()) { // loop through each output
    auto writer = factory.createWriter(writerType, outputName + "_" + statsName);
		...
		attributes.put("informationTextStandalone", "This output was not generated by alsvinn proper, but by a standalone program using the alsvinn libraries");
